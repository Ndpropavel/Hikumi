...
from hikumitl.tl.tlobject import TLObject
...
def patched_import(name: str, *args, **kwargs):
    if name.startswith("telethon"):
        return native_import("hikumitl" + name[8:], *args, **kwargs)

    if name.startswith("pyrogram"):
        return native_import("hikumipyro" + name[8:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        ...
        with contextlib.suppress(AttributeError):
            _hikumi_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?hikumi$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.hikumi_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.hikumi_commands)
                inline_handlers.update(module.hikumi_inline_handlers)
                callback_handlers.update(module.hikumi_callback_handlers)
                watchers.extend(module.hikumi_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__ and instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.hikumi_commands))
            )

        for _command, cmd in instance.hikumi_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.hikumi_commands:
                self.add_alias(alias, cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.hikumi_inline_handlers.copy().items():
        ...
        for name, func in instance.hikumi_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.hikumi_watchers.values():
