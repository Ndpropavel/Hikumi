...
import hikumitl
from hikumitl import events
from hikumitl.errors import (
    ApiIdInvalidError,
...
from hikumitl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from hikumitl.password import compute_check
from hikumitl.sessions import MemorySession, SQLiteSession
from hikumitl.tl.functions.account import GetPasswordRequest
from hikumitl.tl.functions.auth import CheckPasswordRequest
...
class Hikumi:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: str(f).startswith("hikumi-") and str(f).endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.hikumi_db = database.Database(client)
        await client.hikumi_db.init()
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            await client.hikumi_inline.bot.send_animation(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),  # type: ignore
                "https://github.com/hikariatama/assets/raw/master/hikumi_banner.mp4",
                caption=(
                    "🌘 <b>Hikumi {} started!</b>\n\n🌳 <b>GitHub commit SHA: <a"
                    ' href="https://github.com/hikariatama/Hikumi/commit/{}">{}</a></b>\n✊'
                    " <b>Update status: {}</b>\n<b>{}</b>".format(
                        ".".join(list(map(str, list(__version__)))),
                        build,
                        build[:7] if build else "n/a",
            ...
            logging.debug(
                ...
                client.tg_id,
                client.hikumi_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
...
hikumitl.extensions.html.CUSTOM_EMOJIS = not get_config_key("disable_custom_emojis")
