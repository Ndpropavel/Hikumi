...
import grapheme
import hikumitl
import hikumitl.extensions.html as parser
import requests
from aiogram.types import Message as AiogramMessage
from hikumitl import hints
from hikumitl.tl.custom.message import Message
from hikumitl.tl.functions.account import UpdateNotifySettingsRequest
from hikumitl.tl.functions.channels import (
...
from hikumitl.tl.types import TypeMessageEntity as FormattingEntity
from hikumitl.utils import get_input_channel, get_input_chat_photo

from ._internal import fw_protect
from .inline.types import BotInlineCall, BotMessage, InlineCall, InlineMessage
from .tl_cache import CustomTelegramClient
from .types import HikumiReplyMarkup, ListLike, Module
...
def get_chat_id(message: typing.Union[Message, AiogramMessage]) -> typing.Optional[int]:
    ...
    :return: Chat ID
    """
    return hikumitl.utils.resolve_id(
...
def get_entity_id(entity: hints.Entity) -> int:
    ...
    :return: Entity ID
    """
    return hikumitl.utils.get_peer_id(entity)
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[HikumiReplyMarkup] = None,
    **kwargs,
) -> typing.Union[InlineCall, InlineMessage, Message, BotMessage]:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or hikumi inline object
    ...
    parse_mode = hikumitl.utils.sanitize_parse_mode(
        kwargs.pop(
            "parse_mode",
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "hikumi_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "hikumi")
        except Exception:
            folder = None
...
def smart_split(
    ...
    :example:
        >>> utils.smart_split(
            *hikumitl.extensions.html.parse(
                "<b>Hello, world!</b>"
            )
        )
        <<< ["<b>Hello, world!</b>"]
...
def get_commit_url() -> str:
    """
    Get current Hikumi git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/hikariatama/Hikumi/commit/{hash_}">#{hash_[:7]}</a>'
            if hash_
            else "n/a"
        )
    except Exception:
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        (
            hash_,
            f"https://github.com/hikariatama/Hikumi/commit/{hash_}" if hash_ else "",
        )
        if hash_
        else ("", "")
    )
