...
from hikumitl.errors.rpcerrorlist import InputUserDeactivatedError, YouBlockedUserError
...
class InlineManager(
    ...
    :type client: hikumi.tl_cache.CustomTelegramClient
    :type db: hikumi.database.Database
    :type allmodules: hikumi.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("hikumi.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.hikumi_me)
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start hikumi init")
        except (InputUserDeactivatedError, ValueError):
            self._db.set("hikumi.inline", "bot_token", None)
            self._token = False
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start hikumi init"
                )
            except Exception:
