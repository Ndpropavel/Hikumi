...
from hikumitl.errors import FloodWaitError, RPCError
...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.hikumi_me.id
        self._cached_usernames = [
            (
                self._client.hikumi_me.username.lower()
                if self._client.hikumi_me.username
                else str(self._client.hikumi_me.id)
            )
        ]

        self._cached_usernames.extend(
            getattr(self._client.hikumi_me, "usernames", None) or []
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("hikumi-")
            and message.chat.title != "hikumi-logs"
        ):
            if not watcher:
