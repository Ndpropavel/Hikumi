...
import hikumitl
from hikumitl.tl.functions.messages import (
    GetDialogFiltersRequest,
    UpdateDialogFilterRequest,
)
from hikumitl.tl.types import Message
from hikumitl.utils import get_display_name
...
@loader.tds
class HikumiSettings(loader.Module):
    """Advanced settings for Hikumi Userbot"""

    strings = {"name": "HikumiSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"hikumi-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "hikumi" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_hikumi(self, message: Message):
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            hikumitl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.hikumi.web.get_url(proxy_pass=False),
                },
        ...
        if not main.hikumi.web:
            main.hikumi.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.hikumi.api_token,
                proxy=main.hikumi.proxy,
                connection=main.hikumi.conn,
            )
            await main.hikumi.web.add_loader(self._client, self.allmodules, self._db)
            await main.hikumi.web.start_if_ready(
                len(self.allclients),
                main.hikumi.arguments.port,
                proxy_pass=main.hikumi.arguments.proxy_pass,
            )
        ...
        url = await main.hikumi.web.get_url(proxy_pass=True)
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._hikumi_entity_cache)} cache records"
                )
                self._client._hikumi_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._hikumi_fulluser_cache)} cache records"
                )
                self._client._hikumi_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._hikumi_fullchannel_cache)} cache"
                    " records"
                )
                self._client._hikumi_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._hikumi_perms_cache)} cache records"
                self._client._hikumi_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._hikumi_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._hikumi_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._hikumi_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._hikumi_fulluser_cache = {}
                self._client._hikumi_fullchannel_cache = {}
                self._client.hikumi_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._hikumi_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._hikumi_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._hikumi_fullchannel_cache)} records\nLoader"
