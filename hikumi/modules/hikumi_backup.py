...
@loader.tds
class HikumiBackup(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "HikumiBackup"}
    ...
    async def client_ready(self):
        ...
        self._backup_channel, _ = await utils.asset_channel(
            ...
            silent=True,
            archive=True,
            avatar="https://github.com/hikariatama/assets/raw/master/hikumi-backups.png",
            _folder="hikumi",
            invite_bot=True,
        )
    ...
    async def handler(self):
        try:
            ...
            backup.name = (
                f"hikumi-db-backup-{datetime.datetime.now():%d-%m-%Y-%H-%M}.json"
            )
            ...
            await self.inline.bot.send_document(
                ...
                reply_markup=self.inline.generate_markup([[{
                    "text": "↪️ Restore this",
                    "data": "hikumi/backup/restore/confirm",
                }]]),
            )
            ...
            logger.exception("HikumiBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("hikumi/backup/restore"):
            return

        if call.data == "hikumi/backup/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "hikumi/backup/restore",
                },
            )
            return
        ...
        with contextlib.suppress(KeyError):
            decoded_text["hikumi.inline"].pop("bot_token")
