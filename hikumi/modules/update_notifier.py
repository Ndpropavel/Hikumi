...
class UpdateNotifier(loader.Module):
    """Tracks latest Hikumi releases, and notifies you, if update is required"""
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup([
            {"text": self.strings("update"), "data": "hikumi/update"},
            {"text": self.strings("ignore"), "data": "hikumi/ignore_upd"},
        ])
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_animation(
                ...
                caption=self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/hikariatama/Hikumi/compare/{}...{}">{}</a>'.format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"hikumi/update", "hikumi/ignore_upd"}:
            return

        if call.data == "hikumi/ignore_upd":
