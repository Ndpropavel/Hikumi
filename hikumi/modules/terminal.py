...
class MessageEditor:
    def __init__(
        self,
        message: hikumitl.tl.types.Message,
        command: str,
        config,
        strings,
        request_message,
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(hikumitl.errors.rpcerrorlist.MessageNotModifiedError):
            try:
                self.message = await utils.answer(self.message, text)
            except hikumitl.errors.rpcerrorlist.MessageTooLongError as e:
                logger.error(e)
                logger.error(text)
...
class SudoMessageEditor(MessageEditor):
    ...
    async def update_stderr(self, stderr):
        ...
        if lastlines[0] == self.PASS_REQ and self.state == 0:
            ...
            except hikumitl.errors.rpcerrorlist.MessageNotModifiedError as e:
                logger.debug(e)
            ...
            self.message[0].client.add_event_handler(
                ...
                hikumitl.events.messageedited.MessageEdited(chats=["me"]),
            )
    ...
    async def on_message_edited(self, message):
        ...
        if hash_msg(message) == hash_msg(self.authmsg):
            ...
            except hikumitl.errors.rpcerrorlist.MessageNotModifiedError:
...
class RawMessageEditor(SudoMessageEditor):
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(
            hikumitl.errors.rpcerrorlist.MessageNotModifiedError,
            hikumitl.errors.rpcerrorlist.MessageEmptyError,
            ValueError,
        ):
            try:
                await utils.answer(self.message, text)
