...
class Settings(loader.Module):
    ...
    @loader.command()
    async def hikumicmd(self, message: Message):
        await utils.answer_file(
            message,
            "https://github.com/hikariatama/assets/raw/master/hikumi_cat_banner.mp4",
            self.strings("hikumi").format(
                (
                    utils.get_platform_emoji()
                    if self._client.hikumi_me.premium and CUSTOM_EMOJIS
                    else "🌘 <b>Hikumi userbot</b>"
                ),
                *version.__version__,
                utils.get_commit_url(),
                f"{hikumitl.__version__} #{hikumitl.tl.alltlobjects.LAYER}",
            )
            + (
                ""
