...
import requests
from hikumitl.errors.rpcerrorlist import MediaCaptionTooLongError
from hikumitl.tl.functions.channels import JoinChannelRequest
from hikumitl.tl.types import Channel, Message, PeerUser
...
class Loader(loader.Module):
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?hikumi_min", doc):
            ver = re.search(r"# ?scope: ?hikumi_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        module_name = f"hikumi.modules.{uid}"
        doc = geek.compat(doc)
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "hikumi_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.hikumi_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.hikumi_entity_cache
                    and getattr(
