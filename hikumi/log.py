...
def getlines(filename: str, module_globals=None) -> list[str]:
    ...
    source code of Hikumi modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (module.startswith("hikumi.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        if (
                            ...
                            value,
                            (hikumitl.TelegramClient, CustomTelegramClient),
                        ):
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.hikumi_caller or client_id == record.hikumi_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        item: HikumiException,
    ):
        chunks = item.message + "\n\n<b>🪐 Full traceback:</b>\n" + item.full_stack

        chunks = list(utils.smart_split(*hikumitl.extensions.html.parse(chunks), 4096))  # type: ignore
    ...
    def _gen_web_debug_button(self, item: HikumiException) -> list:
        if not item.sysinfo:
            return []
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], HikumiException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_hikumi_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_hikumi_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        record.hikumi_caller = caller

        if record.levelno >= self.tg_level:
            if record.exc_info:
                exc = HikumiException.from_exc_info(
                    *record.exc_info,
...
def init():
    ...
    logging.getLogger("hikumitl").setLevel(logging.WARNING)
