...
- `NHeta`
- Add HikumiHost as a platform
- Update to Hikumi-TL 2.0.7
- Allow double-restart
- Personal messages scam modules protection
- https://github.com/hikariatama/Hikumi/issues/115
- https://github.com/hikariatama/Hikumi/issues/114
- https://github.com/hikariatama/Hikumi/issues/119
- https://github.com/hikariatama/Hikumi/issues/120
...
- Update `hikumi-tl` in accordance to upstream telethon v1
- Update `hikumi-pyro` in accordance to upstream pyrogram
...
- Drop Okteto support
- Show Hikumi platform and version in Telegram sessions list
...
- Fix `hikumi.types.CoreOverwriteError`
...
- Add `hikumi.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in hikumi inline
...
- Change inline query placeholder to `user@hikumi:~$` + legacy migration
...
- Create new type :obj:`hikumi.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`hikumi.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`hikumi.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Hikumi-TL >= 1.24.9
...
- Publish hikumi telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `HikumiSecurity`
- Fix `.uninstall_hikumi` being accessible by sudo
- Fix `utils.find_caller` for :method:`hikumi.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Hikumi emojis to premium users
...
- Move `hikumi._types` to `hikumi.types` with legacy support
- Remake all core modules to decorators
- Force custom hikumi telethon installation with 144 layer support
...
- Add support for `# scope: hikumi_min` metatag in libraries
...
- Update quickstart
- New fields in HikumiInfo
- Remove suffix `Hikumi` if user specified it somewhere below
...
- Fix processing of `# scope: hikumi_min`
...
- Add `banner_url` config var to `HikumiInfo`
...
- Add `client.force_get_entity` to bypass Hikumi Cacher
...
- Fix translation issue in HikumiConfig
- Fix `.dump`
...
- Add banner with Hikumi installation status
...
- Return :obj:`InlineMessage` in `hikumi.inline.gallery.Gallery.gallery`
...
- Add heroku waker
- Make `hikumi-` app naming optional
...
- Merge `hikumi_logger.py` with `test.py`
- Localization
...
- Replace .uninstall_hikumi with full uninstallation (remove bot, asset chats and folder)
...
- Add automatic hikumi folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to hikumi folder)
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Hikumi/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Hikumi 1.1.0

- Make inline commands' docstrings translatable
- Introducing HikumiDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor HikumiSecurity, especially the bounding mask control
...
- Add ability to disable HikumiDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Hikumi 1.0.22

Add HikumiDL module to use inline download buttons in verified channels
