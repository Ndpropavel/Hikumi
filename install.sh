...
runin() {
	...
	{ "$@" 2>>../hikumi-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../hikumi-install.log
	done
}
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" hikumi-install.log
fi

if [ -d "Hikumi/hikumi" ]; then
	cd Hikumi || {
...
if [ -f ".setup_complete" ]; then
	# If hikumi is already installed by this script
	PYVER=""
...
echo "Installing..." >hikumi-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/hikariatama/Hikumi/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Hikumi
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/hikariatama/Hikumi/ || {
